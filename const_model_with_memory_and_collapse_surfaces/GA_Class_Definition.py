# -*- coding: utf-8 -*-
"""
Created on Wed Apr 14 21:38:23 2021
...
###2.2 计算适应度函数值
    ...
    def fitness_solve(self,true_params_value,exp_data_strain,exp_data_stress):
        ...
        for i in range(self.population_size):
            origin_value,weight_value = goal_function(theory_stress_population[i],exp_data_stress,exp_data_strain)
            ## origin_value是平权的目标函数值，weight_values是加权目标函数值； 对于求最小值问题，可计算倒数作为适应度指标
            origin_goal_value.append(origin_value)
            weight_goal_value.append(weight_value)
            
            fitness_value.append(1/weight_goal_value[i])           ###fitness_value[]:list[]
        return origin_goal_value,weight_goal_value,fitness_value
...
### 2.6 找出当前种群中最好的适应度和对应的参数值
    ...
    def worst_find(self,true_params_value,fitness_value):
        ...
        worstparameters = []
        worstfitness = 500000    ## 选一个较大的值作为初始值
