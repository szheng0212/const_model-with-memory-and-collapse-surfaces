# -*- coding: utf-8 -*-
"""
Created on Sun Apr 25 21:15:42 2021
...
class ChabocheModel1D(object):
    ...
    def step_solve(self, teps_n1,teps_n):
        ...
        if (F_yield <= ERR): # 判断为弹性状态
            ...
            dpn1 = NewtonIteration(a1,a2,a3,a4,self.c1,self.c2,self.c3,self.c4,self.r1,self.r2,self.r3,self.r4,\
                    self.G,self.sgm0,self.Q1,R1,self.bios1,self.Q2,R2,self.bios2,R3,self.bios3,stress_trial)      # 调用dp求解函数迭代求解该加载步的dp
            ...
            S_a = (self.sgm0 + R) * abs(stress_trial_n1) * np.sign(stress_trial_n1) / ((self.sgm0 + R) + \
                    (3 * self.G + theta1 * self.c1 + theta2 * self.c2 + theta3 * self.c3 + theta4 * self.c4) * dpn1)
