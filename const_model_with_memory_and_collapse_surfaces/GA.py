...
from ChaobocheModel1D import ChabocheModel1D
...
###2.2 计算适应度函数值
    def translation(self,population):           # 将二进制位串转化为十进制数字
        ...
        for i in range(len(population)):            ## 遍历种群数（所有个体）
            ...
            for j in range(len(population[0])):         # 遍历每条染色体上的所有基因，即待优化参数的个数
                total = int(0)                          ## 暂存器，存储一条染色体上的所有基因值，即一套参数的数值
                for l in range(len(population[0][0])):  # 遍历每个基因的位串长度，即单个基因的编码长度
                    total = int(total + population[i][j][l] *\
    ...
    def true_params_solve(self,population):     ## 将上面的基因表现型十进制数字转换为考虑单参数上下限数值的 参数真实数值
        ...
        for i in range(len(population)):         # 遍历每代的所有染色体（个体） （在此认为每条染色体即为一个个体）
            ...
            for j in range(len(population[0])):        # 遍历染色体上的每个基因 第j个基因值
                value = float(self.LB[j]+population_decimalism[i][j] *\
                          (self.UB[j]-self.LB[j]) / \
    ...
    def population_stress_solve(self,population,exp_data_strain):        ## 接受一个exp_data_strain 即试验数据 应力列表
        """
        ...
        true_params_value = self.true_params_solve(population)  ##  调用true_params_solve()获取该代种群所有个体的 真实数值 list[][]
    ...
    def fitness_solve(self,population,exp_data_strain,exp_data_stress):
        fitness_value = []
        theory_stress_population = self.population_stress_solve(population,exp_data_strain)     ## theory_stress_population[][]
        for i in range(len(population)):
...
###2.3 选择操作
    ...
    def cumsum(self,fitness1):      ## 使用轮盘赌方法 选择 selection operator
        ...
        '''
        ##计算适应度函数值累加列表
        for i in range(len(fitness1)-1,-1,-1):            # range(start,stop,[step]) #     倒计数：从最后一位迭代至第一位
            total = 0.0
            j=0
            while(j<=i):
                ...
                j = j + 1
            fitness1[i] = total
        #return fitness1     ## 仅仅为了测试返回情况，无意义## 此时fitness1[]经过函数运算已经发生变化
    ...
    def selection(self,population,fitness_value):
        ...
        self.cumsum(new_fitness)    ## 此时new_fitness已经是适应度累计值列表
        ...
        new_population = population     ## 将population[][][]传至新的列表 子代列表
        
        ## 轮盘赌方式选择染色体
        while new_in < pop_len & fit_in < pop_len:
            if(ms[new_in] < new_fitness[fit_in]):             ## 判断是否被选入交配池
                for j in range(len(population[0])):         ## 遍历个体中的所有参数，写入新的子代new_population
                    new_population[new_in][j]=population[fit_in][j]
                new_in = new_in + 1
...
### 2.4 交叉操作        
    def crossover(self,population):
        ...
        for i in range(params_num):
            for j in range(0,len(population)-1,2): ## 遍历所有参数遍历群体中所有个体,考虑到最后一位发生越界，故pop_num-1
            ## 在此认为在一轮交叉操作过程中，同一条染色体最多只能参与一次交叉操作！！
            ## 第j个染色体的第i个参数 
                if (random.random() < self.pc):                    
                    cpoint = random.randint(0,len(population[j][i])) ## 随机选择基因中的交叉点 在0和单个基因的位串数之间随机选择一个整数
                    ###实现相邻的染色体基因取值的交叉                    
                    tmp1 = []
                    tmp2 = []
                    #将tmp1作为暂存器，暂时存放 第j个染色体 第i个取值   中的    前0到cpoint个基因，
                    #然后再把   第j+1个染色体 第i个取值 中的后面的基因，补充到tem1后面
                    tmp1.extend(population[j][i][0:cpoint])     ## tmp1和tmp2 存储了 某个参数基因上面的 位串信息（序列）
                    tmp1.extend(population[j+1][i][cpoint:len(population[j][i])])
                    #将tmp2作为暂存器，暂时存放第j个染色体第i个取值中的前0到cpoint个基因，
                    #然后再把第j个染色体第i个取值中的后面的基因，补充到tem2后面
                    tmp2.extend(population[j+1][i][0:cpoint])
                    tmp2.extend(population[j][i][cpoint:len(population[j][i])])
                    #将交叉后的染色体取值放入新的种群中
                    population[j][i] = tmp1     ## 将交叉后的位串赋值给 参数 ，形成新的子代
                    population[j+1][i] = tmp2
### 2.5 变异操作
...
### 2.6 找出当前种群中最好的适应度和对应的参数值
    def best_find(self,true_params_value,fitness_value):
        ...
        for i in range(0,pop_len):  ## 遍历种群中所有个体（染色体）
            ...
            if (fitness_value[i] > bestfitness):
                ...
                pass
        return bestparameters,bestfitness   ## 返回给当前代的最佳适应度值的个体参数组合，及最佳适应度值
...
### 2.7 画出适应度函数值变化图
    def plot(self,results):
        ...
        plt.plot(X,Y)
        plt.xlabel('Number of iteration',size = 50)
        plt.ylabel('The Best Value of Fitness',size = 50)
        plt.title('Constitutive_Model Parameters Optimization Based GA Method')     ## 绘制的是每代迭代最佳个体的适应度值随迭代次数的关系曲线
        plt.show()
...
if __name__ == '__main__':
    ...
    table = xl.sheets()[0]                                    
    test_strain = table.col_values(1)                              # 读取数据列 应变 列表list
    test_stress = table.col_values(2)                              # 读取数据列 应力 列表list
