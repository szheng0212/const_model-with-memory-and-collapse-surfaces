...
###2.2 计算适应度函数值
    def true_params_solve(self,population):
...
###2.3 选择操作
    ...
    def cumsum(self,fitness1):      ## 使用轮盘赌方法 选择 selection operator
        ...
        output:适应度函数值累加列表
        '''
        tmp = fitness1 #存储累加列表，防止cumsum改变fitness1的值
        ...
        for i in range(len(tmp)-1,-1,-1):            # range(start,stop,[step]) #     倒计数：从最后一位迭代至第一位
            ...
            while(j<=i):
                total = total + tmp[j]
                j = j + 1
            tmp[i] = total         ## 防止了函数cumsum改变参数fitness1的值
