...
'''
import numpy as np
from Newton_Raphson_iteration import NewtonIteration
...
class ChabocheModel1D(object):
    ...
    def step_solve(self, teps_n1,teps_n):
        ...
        # stress_trial = self.E * (teps_n1-peps)                   # 计算第 n+1步 试应力 n+1步总应变-n步塑性应变
        ERR = 0.000001


        if (abs(stress_trial - a) - self.sgm0 - R <= ERR):    # 判断为弹性状态
            sgm = stress_trial
                                                    # 弹性域增量
                                                    # a1增量
                                                    # a2增量
            ...
            dp = NewtonIteration(a1,a2,a3,self.c1,self.c2,self.c3,self.r1,self.r2,self.r3,\
                self.G,self.sgm0,self.Q,R,self.bios,stress_trial)      # 调用dp求解函数迭代求解该加载步的dp
                    # 由dp值，分别更新 dpeps，peps，da1，da2，da3，a1，a2，a3，a，dR，R，sgm
            dpeps = np.sign(stress_trial - a1 -a2 -a3) * dp  ## 计算这一步的塑性应变增量
            ...
            sgm = sgm + self.E * (teps_n1 - teps_n - dpeps) # 根据第n+1步的弹性应变更新应力，而不是直接按照总的计算，好像可以减少步计算的误差???
